...
func NewMarkdownRouter(console slog.Handler, dir string) (*MarkdownRouter, error) {
	...
	specs := []struct {
		...
		{categoryChainPool, chainPoolFile, "链上博弈池监控日志"},
		{categoryAIManaged, aiManagedFile, "AI 自动托管日志"},
		{categoryAIOracle, aiOracleFile, "多 AI 裁决开奖日志"},
	...
	for _, spec := range specs {
		...
		if info.Size() == 0 {
			header := fmt.Sprintf("%s\n\n# %s\n\n_本次后端运行 · Asia/Shanghai · 每个事件单独展示_\n", formatMarker, spec.title)
